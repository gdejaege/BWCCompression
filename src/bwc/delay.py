...
class Delay(Windowed):
    ...
    def next_window(self, time):
        ...
        for tid, point in self.last_points.items():
            point.priority = float("inf")
            self.priority_list.add(point)
            self.window_trips.setdefault(tid, []).append(point)
        self.last_points = dict()
    ...
    def compress(self):
        ...
        # keep points of window
        self.next_window(window_end)
        # keep last_points which got just inserted in priority queue
        self.next_window(window_end)
        self.finalize_trips()
